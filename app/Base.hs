...
-- Ethernet State
data EthernetState = EthernetState Bool Net.IPv4 Net.IPv6 deriving (Show)

data State = State
  { i3State :: Maybe I3State,
    timeState :: Maybe TimeState,
    ethernet :: Maybe EthernetState
  }
  deriving (Show)
...
updateI3 _ (I3PrivateState socket) sharedState pingChannel = do
...
updateTime _ sharedState pingChannel = do
...
updateEthernet :: [ItemConfig] -> SharedState -> PingChannel -> IO ()
updateEthernet configs sharedState pingChannel = do
  interfaces <- Net.getNetworkInterfaces

  modifyMVar_ sharedState $ \state -> return $ state {ethernet = Just $ EthernetState False Nothing Nothing}
  sendPing pingChannel
  threadDelay $ seconds 30
